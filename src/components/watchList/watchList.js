...
const WatchList = ({ index, watchlists }) => {
  ...
  const {
    removeWatchlist: removeWatchListDialog,
    dispatch: dispatchDialogContext,
  } = useDialogContext();
  ...
  return (
    <TableRow
      ...
      <TableCell className="row-tbl-coin">{watchlists["cryptoid"]}</TableCell>

      <TableCell className="row-tbl-coin" align="right">
        <button
          className="btn-coin-watchlist"
          onClick={async (e) => {
            e.stopPropagation();
            await handleDeleteWatchLists(watchlists["favid"]);
          }}>
          Remove
        </button>
      </TableCell>
